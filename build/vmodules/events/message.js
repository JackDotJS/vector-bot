...
exports.run = async (message) => {
    ...
    if (message.author.bot || message.author.system || message.system || message.type !== `DEFAULT`)
        return;
    // bot is in debug mode and the message author is not whitelisted
    if (bot.debug && ![bot.cfg.dev.discord_user_id, ...bot.cfg.dev.user_whitelist].includes(message.author.id))
        return;
    ...
    if (valid_cmd)
        return handle_command(bot, message, gcfg, perms);
};
async function handle_command(bot, message, gcfg, perms) {
    ...
    const unknown = async () => {
        const embed = new discord_js_1.default.MessageEmbed()
            .setAuthor(bot.managers.locale.text(`cmd.unknown.title`, gcfg.lang), await bot.managers.assets.getIcon(`info`, bot.cfg.colors.default))
            .setFooter(bot.managers.locale.text(`cmd.unknown.help`, gcfg.lang, gcfg.commands.prefixes[0] + `help`))
            .setColor(bot.cfg.colors.default);
        if (shard_memory_js_1.default.commands.length === 0)
            return message.reply({ embeds: [embed] });
        const ratings = [];
        for (const searchcmd of shard_memory_js_1.default.commands) {
            if (searchcmd.guilds != null && searchcmd.guilds.includes(message.guild.id))
                continue;
            if (gcfg.commands.hidden.includes(searchcmd.name))
                continue;
            if (gcfg.commands.disabled.includes(searchcmd.name))
                continue;
            ratings.push({
                name: searchcmd.name,
                distance: (0, jaro_winkler_1.default)(input_cmd, searchcmd.name)
            });
        }
        ratings.sort((a, b) => b.distance - a.distance);
        if (ratings[0].distance > 0.5) {
            const desc = bot.managers.locale.text(`cmd.unknown.suggestion`, gcfg.lang, gcfg.commands.prefixes[0] + ratings[0].name, (ratings[0].distance * 100).toFixed(1));
    ...
    try {
        ...
        const file_rgx = new RegExp(`${path_1.default.parse(process.cwd()).root}\\[^,]+\\d+:\\d+`);
